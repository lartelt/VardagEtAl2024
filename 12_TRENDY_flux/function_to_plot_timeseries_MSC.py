...
def calculateMeanSeasonalCycle_of_fluxes(Flux_per_subregion_monthly_timeseries: pd.DataFrame, model: str='MIP', TRENDY_FLUXCOM_variable: str=None):
    ...
    if model=='MIP' or model=='MIP_ens':
        ...
        Flux_msc = Flux_per_subregion_monthly_timeseries.groupby(['Month'])[['tp_mean']].mean().reset_index()
        Flux_msc_std = Flux_per_subregion_monthly_timeseries.groupby(['Month'])[['tp_mean']].std(ddof=0).reset_index()
        Flux_msc_count = Flux_per_subregion_monthly_timeseries.groupby(['Month'])[['tp_mean']].count().reset_index()
        Flux_msc = Flux_msc.merge(Flux_msc_std, on='Month', suffixes=('', '_std'))
        Flux_msc = Flux_msc.merge(Flux_msc_count, on='Month', suffixes=('', '_count'))
        return Flux_msc
...
def plot_timeseries_of_TRENDY_fluxes(df: pd.DataFrame,
    ...
    # arguments:
        ...
        - compare_case:
            1: Only plot TRENDY timeseries with mean & std
            2: plot TRENDY & TM5/IS+ACOS
            3: TRENDY mean w/ std & TM5/IS+ACOS & TM5/IS+RT
            4: Only plot TRENDY timeseries with mean withOUT std
    '''
    ...
    if compare_case==1:
        ...
        for i, column in enumerate(df.columns):
            if column=='MonthDate' or column=='Year' or column=='Month' or column=='mean' or column=='std' or column=='count':
                ...
                print(column)
                print(column[17:])
                ax1.plot(df.MonthDate, df[column], color = color_dict[(column[17:])], linestyle=linestyle_dict[(column[17:])], label=(column[17:]), linewidth=1, zorder=i+1)
                print(i)
        ax1.plot(df.MonthDate, df['mean'], color = 'black', linestyle='dashed', label='mean TRENDY', linewidth=1.2, zorder=len(df.columns)+2)
        #ax1.plot(df_TM5_ACOS.MonthDate, df_TM5_ACOS['CO2_NEE_flux_monthly_TgC_per_subregion'], color = 'firebrick', linestyle='solid', label='TM5-4DVar/IS+GOSAT', linewidth=1.2)
    ...
    if compare_case==4:
        ax1.legend(framealpha=0.25, facecolor='grey', loc='upper left', fontsize=9.5, ncol=6, columnspacing=0.6, bbox_to_anchor=(-0.13, 1.3))
        ax1.set_title('TRENDY '+what_is_plotted+' fluxes & mean '+region_name)
        plt.savefig(savepath+'timeseries_all_TRENDY_models_with_mean_supplemental', dpi=300, bbox_inches='tight')
...
def plot_timeseries_MSC_of_TRENDY_variable_fluxes(df_TRENDY: pd.DataFrame,
                                             ...
                                             df_TM5_ACOS: pd.DataFrame=None,
                                             df_TM5_RT: pd.DataFrame=None,
                                             plot_mean_TM5_ACOS_RT: bool=False,
                                             df_mean_TM5_ACOS_RT: pd.DataFrame=None,
    ...
    if compare_case==1 or compare_case==2:
        ...
        if plot_TM5_ACOS:
            ...
            ax2.fill_between(MSC_GFED.Month,
                             ...
                             MSC_GFED['total_emission'] + MSC_GFED[('total_emission'+'_std')], 
                             color = 'black', linestyle='solid', alpha=0.2)
        elif plot_mean_TM5_ACOS_RT:
            ax1.plot(df_mean_TM5_ACOS_RT.MonthDate, df_mean_TM5_ACOS_RT['CO2_NEE_fire_flux_monthly_TgC_per_subregion'], color = 'black', linestyle='dashed', label='TM5-4DVar/IS+GOSAT', linewidth=1.25)
            MSC_TM5 = calculateMeanSeasonalCycle_of_fluxes(df_mean_TM5_ACOS_RT, model='TM5')
            MSC_TM5.loc[:first_month,'Month'] += 12
            MSC_TM5_new = pd.DataFrame(pd.concat([MSC_TM5.loc[first_month+1:, 'Month'], MSC_TM5.loc[:first_month, 'Month']], ignore_index=True))
            MSC_TM5 = pd.merge(MSC_TM5_new, MSC_TM5, on='Month', how='outer')
            ax2.plot(MSC_TM5.Month, MSC_TM5['CO2_NEE_fire_flux_monthly_TgC_per_subregion'], color = 'black', linestyle='dashed', label = 'TM5-4DVar/IS+ACOS', linewidth=1.25)
...
def plotSumOfFluxesBarChart(df_trendy_fluxes: list=[],
    ...
    if compare_case==3:
        print('Case 3')
        for i, column in enumerate(df_trendy_fluxes):
            df_trendy_fluxes_sum = column.groupby(['Year'])['mean'].sum().reset_index()    
            b = ax.bar(df_trendy_fluxes_sum['Year'] + (i)*bar_width, (df_trendy_fluxes_sum['mean']), bar_width, alpha=opacity, color=color_list[i], label=label_list[i])
            savename = savename + '_' + label_list[i]
            del df_trendy_fluxes_sum
    print(j)
...
def plot_timeseries_MSC_of_mean_of_different_model_categories_NEW(df_TRENDY: pd.DataFrame,
                                                  ...
                                                  plot_TM5_ACOS_flux: bool=False,
                                                  df_TM5_ACOS: pd.DataFrame=None,
                                                  df_TM5_IS_GOSAT: pd.DataFrame=None,
                                                  df_TM5_RT: pd.DataFrame=None,
                                                  plot_FXC: bool=False,
                                                  df_FXC: pd.DataFrame=None,
                                                  set_ax_limit: bool=False,
                                                  error_type: str='std_of_models',
                                                  color_list: list=['forestgreen', 'midnightblue', 'darkorange'],
                                                  linestyle_list: list=['solid', 'solid', 'solid'],
    ...
    if compare_case==1 or compare_case==2:
        if plot_TM5_ACOS_flux:
            if plot_MSC_only==False:
                #ax1.plot(df_TM5_ACOS.MonthDate, df_TM5_ACOS['CO2_NEE_fire_flux_monthly_TgC_per_subregion'], color = 'black', linestyle='dashed', label='TM5-4DVar/IS+ACOS', linewidth=1.25)
                #ax1.plot(df_TM5_RT.MonthDate, df_TM5_RT['CO2_NEE_fire_flux_monthly_TgC_per_subregion'], color = 'dimgrey', linestyle='dashed', label='TM5-4DVar/IS+RT', linewidth=1.25)
                ax1.plot(df_TM5_IS_GOSAT.MonthDate, df_TM5_IS_GOSAT['CO2_NEE_fire_flux_monthly_TgC_per_subregion'], color = 'black', linestyle='dashed', label='TM5-4DVar/IS+GOSAT', linewidth=1.25)
            #MSC_TM5 = calculateMeanSeasonalCycle_of_fluxes(df_TM5_ACOS, model='TM5')
            MSC_TM5 = calculateMeanSeasonalCycle_of_fluxes(df_TM5_IS_GOSAT, model='TM5')
            MSC_TM5.loc[:first_month,'Month'] += 12
            MSC_TM5_new = pd.DataFrame(pd.concat([MSC_TM5.loc[first_month+1:, 'Month'], MSC_TM5.loc[:first_month, 'Month']], ignore_index=True))
            MSC_TM5 = pd.merge(MSC_TM5_new, MSC_TM5, on='Month', how='outer')
            ax2.plot(MSC_TM5.Month, MSC_TM5['CO2_NEE_fire_flux_monthly_TgC_per_subregion'], color='black', linestyle='dashed', label = 'TM5-4DVar/IS+ACOS', linewidth=1.25)
        ...
        if plot_FXC==True:
            print('Plotting FXC...')
            if plot_MSC_only==False:
                ax1.plot(df_FXC.MonthDate, df_FXC['NBPtot'], color = 'purple', linestyle='solid', label='FXC NEE+GFED', linewidth=1.25)
            #include std of the here plotted TRENDY mean
            MSC = calculateMeanSeasonalCycle_of_fluxes(df_FXC, model='FXC', TRENDY_FLUXCOM_variable='NBPtot')
            MSC.loc[:first_month,'Month'] += 12
            MSC_new = pd.DataFrame(pd.concat([MSC.loc[first_month+1:, 'Month'], MSC.loc[:first_month, 'Month']], ignore_index=True))
            MSC = pd.merge(MSC_new, MSC, on='Month', how='outer')
            ax2.plot(MSC.Month, MSC['NBPtot'], color='purple', linestyle='solid', label='FXC NEE+GFED', linewidth=1.25)
            ax2.fill_between(MSC.Month,
                                MSC['NBPtot'] - MSC[('NBPtot'+'_std')], MSC['NBPtot'] + MSC[('NBPtot'+'_std')], 
                                color='purple', linestyle='solid', alpha=0.2)
        ...
        if plot_MSC_only==False:
            ...
            ax1.plot(df_TRENDY_modified_very_good.MonthDate, df_TRENDY_modified_very_good['mean'], color=color_list[0], linestyle=linestyle_list[0], label='selected strict models', linewidth=1.25)
            ...
            ax1.plot(df_TRENDY_modified_ok.MonthDate, df_TRENDY_modified_ok['mean'], color=color_list[1], linestyle=linestyle_list[1], label='selected loose models', linewidth=1.25)
            ...
            ax1.plot(df_TRENDY_modified_bad_low.MonthDate, df_TRENDY_modified_bad_low['mean'], color=color_list[2], linestyle=linestyle_list[2], label='other low ampl models', linewidth=1.25)
            ...
            ax1.plot(df_TRENDY_modified_bad_high.MonthDate, df_TRENDY_modified_bad_high['mean'], color=color_list[3], linestyle=linestyle_list[3], label='other high ampl models', linewidth=1.25)
        
        MSC_mean_very_good = calculateMeanSeasonalCycle_of_fluxes(df_TRENDY_modified_very_good, model='TRENDY', TRENDY_FLUXCOM_variable='mean')
        MSC_std_very_good = calculateMeanSeasonalCycle_of_fluxes(df_TRENDY_modified_very_good, model='TRENDY', TRENDY_FLUXCOM_variable='std')
        MSC_mean_ok = calculateMeanSeasonalCycle_of_fluxes(df_TRENDY_modified_ok, model='TRENDY', TRENDY_FLUXCOM_variable='mean')
        MSC_std_ok = calculateMeanSeasonalCycle_of_fluxes(df_TRENDY_modified_ok, model='TRENDY', TRENDY_FLUXCOM_variable='std')
        MSC_mean_bad_low = calculateMeanSeasonalCycle_of_fluxes(df_TRENDY_modified_bad_low, model='TRENDY', TRENDY_FLUXCOM_variable='mean')
        MSC_std_bad_low = calculateMeanSeasonalCycle_of_fluxes(df_TRENDY_modified_bad_low, model='TRENDY', TRENDY_FLUXCOM_variable='std')
        MSC_mean_bad_high = calculateMeanSeasonalCycle_of_fluxes(df_TRENDY_modified_bad_high, model='TRENDY', TRENDY_FLUXCOM_variable='mean')
        MSC_std_bad_high = calculateMeanSeasonalCycle_of_fluxes(df_TRENDY_modified_bad_high, model='TRENDY', TRENDY_FLUXCOM_variable='std')
    ...
    if plot_MSC_only==False:
        ...
        ax1.grid(visible=True, which='minor', color='white', axis='both', linestyle='-', linewidth='0.4')
        ax1.tick_params(bottom=True, left=True, color='gray')
        ax1.legend(framealpha=0.7, facecolor='white', loc='lower left', fontsize=9.5, ncol=2, columnspacing=0.6)
        ax1.set_title('TRENDY '+variable_to_plot+' mean flux of good & bad models '+region_name)#, pad=-13)
        #ax1.legend(framealpha=0.25, facecolor='grey', loc='upper left', fontsize=9.5, ncol=6, columnspacing=0.6, bbox_to_anchor=(-0.13, 1.3))
        ax2.set_xticks([5,10,15])
        ax2.set_xticklabels([5, 10, 3])
    ...
    if compare_case==1:
        ...
        if plot_MSC_only:
            if plot_TM5_ACOS_flux:
                plt.savefig(savepath+'/'+str(start_year)+'-'+str(end_year)+'/supplemental/NEW_TRENDY_MSC_only_'+variable_to_plot+'_mean_of_very_good_ok_bad_low_bad_new_high_new_model_flux_AND_TM5_IS+ACOS_RT.png', dpi=300, bbox_inches='tight')
            else:
                plt.savefig(savepath+'/'+str(start_year)+'-'+str(end_year)+'/supplemental/NEW_TRENDY_MSC_only_'+variable_to_plot+'_mean_of_very_good_ok_bad_low_bad_new_high_new_model_flux.png', dpi=300, bbox_inches='tight')
        else:
            if plot_TM5_ACOS_flux and plot_FXC:
                #plt.savefig(savepath+'/'+str(start_year)+'-'+str(end_year)+'/supplemental/NEW_TRENDY_timeseries_MSC_'+variable_to_plot+'_mean_of_very_good_ok_bad_low_new_bad_high_new_model_flux_AND_TM5_IS+ACOS_RT_AND_FXC.png', dpi=300, bbox_inches='tight')
                plt.savefig(savepath+'/'+str(start_year)+'-'+str(end_year)+'/supplemental/NEW_TRENDY_timeseries_MSC_'+variable_to_plot+'_mean_of_very_good_ok_bad_low_new_bad_high_new_model_flux_AND_TM5_IS+ACOS_RT.png', dpi=300, bbox_inches='tight')
            else:
                plt.savefig(savepath+'/'+str(start_year)+'-'+str(end_year)+'/supplemental/NEW_TRENDY_timeseries_MSC_'+variable_to_plot+'_mean_of_very_good_ok_bad_low_new_bad_high_new_model_flux.png', dpi=300, bbox_inches='tight')
    print('Done saving figure!')
...
def plot_timeseries_MSC_general(df_list: list=[],
    ...
    ax2.get_xaxis().set_minor_locator(mpl.ticker.AutoMinorLocator())
    ax2.get_yaxis().set_minor_locator(mpl.ticker.AutoMinorLocator())
    ax2.tick_params(bottom=True, left=False, color='gray')
    if plot_MSC_only:
        ax2.set_ylabel('CO$_2$ flux [TgC/month/region]', color='black')
        if norm_timeseries:
...
# plot bar chart sum of fluxes
def plotSumOfFluxesBarChart_general(df_trendy_list: list=[],
                            df_TM5_IS_GOSAT_sum_fluxes: pd.DataFrame=None,
                            df_FLUXCOM_sum_fluxes: pd.DataFrame=None,
                            variable_plotted: str='nbp',
                            color_list: list=[],
                            label_list: list=[],
                            hatch_list: list=None,
    ...
    # arguments
        compare_case:
            1: plot TRENDY fluxes & TM5/ACOS NEE+fire fluxes
        hatch_list: list of what hatches should be plotted
    '''
    fig, ax = plt.subplots()
    if df_FLUXCOM_sum_fluxes is not None:
        bar_width = 1/(len(df_trendy_list)+3)
    else:
        bar_width = 1/(len(df_trendy_list)+2)
    opacity = 1
    variable_length=len(variable_plotted)
    savename = 'TRENDY_bar-chart_'+variable_plotted
    year_list = np.arange(2010, 2019, 1)
    j=0
    if compare_case==1:
        print('Case 1')
        a = ax.bar(df_TM5_IS_GOSAT_sum_fluxes['year'] + (j)*bar_width, (df_TM5_IS_GOSAT_sum_fluxes['CO2_NEE_fire_flux_monthly_TgC_per_subregion']), bar_width, alpha=opacity, color='firebrick', hatch='///', label='TM5-4DVar/IS+GOSAT')
        savename = savename + '_TM5_IS_GOSAT_NEE_fire'
        j=j+1
        if df_FLUXCOM_sum_fluxes is not None:
            c = ax.bar(df_FLUXCOM_sum_fluxes['Year'] + (j)*bar_width, (df_FLUXCOM_sum_fluxes['NBPtot']), bar_width, alpha=opacity, color='purple', hatch='///', label='FXC NEE+GFED')
            j=j+1
            savename = savename + '_FXC_NEE_GFED'
        for i, column in enumerate(df_trendy_list):
            print(j)
            #length_models_plotted=length_models_plotted+1
            df_trendy_fluxes_sum = column.groupby(['Year'])[['mean']].sum().reset_index()
            b = ax.bar(df_trendy_fluxes_sum['Year'] + (j)*bar_width, (df_trendy_fluxes_sum['mean']), bar_width, alpha=opacity, color=color_list[i], label=label_list[i])
            savename = savename + '_' + (label_list[i].replace(' ', '_')).replace('/','_')
            j=j+1
            del df_trendy_fluxes_sum
    print(j)
    ax.set_xticks(year_list + ((j-1)/2)*bar_width)
    ...
    ax.set_xlabel('Year')
    ax.set_ylabel('Carbon flux [TgC/region/year]', color='black')
    ax.set_title('TRENDY yearly sum of '+variable_plotted+' fluxes ' + region_name)
    ax.set_axisbelow(True)
    ax.axhline(y=0, xmin=0, xmax=1, color='grey', linewidth='0.9')
    ax.minorticks_on()
    #ax.grid(which='minor', color='grey', axis='x', linestyle='-', linewidth='0.1')
    ax.grid(which='major', color='grey', axis='x', linestyle='-', linewidth='0.75', alpha=1)
    if df_FLUXCOM_sum_fluxes is not None:
        ax.legend(framealpha=0.9, fontsize=9.5, loc='lower left', ncol=1, columnspacing=0.6, handletextpad=0.4)
    else:
        ax.legend(framealpha=0.5, fontsize=9.5, loc='lower left', ncol=1, columnspacing=0.6, handletextpad=0.4)
    plt.savefig(savepath + savename + '_' + region_name + '.png', dpi=300, bbox_inches='tight')
    #plt.savefig('/home/lartelt/MA/software/12_TRENDY_flux/plot_bar-charts/test.png', dpi=300, bbox_inches='tight')
    print('Done saving plot!')
...
# plot timeseries & MSC GENERAL with BAR plot as subplot under the original plot
def plot_MSC_with_bar_subplot(df_list: list=[],
                              model_list: list=['FLUXCOM', 'TRENDY'],
                              columns_to_plot: list=['TERtot', 'mean'],
                              columns_to_plot_std: list=['TER_madtot', 'std'],
                              columns_to_plot_label: list=['TER', 'ra+rh'],
                              ...
                              color_list = ['black', 'forestgreen'],
                              linestyle_list = ['solid', 'dashed'],
                              df_for_bar_plot: list=[],
                              column_to_plot_bar_plot: list=['tp'],
                              columns_to_plot_label_bar_plot: list=['precip'],
                              start_month: int=5,
                              start_year: int=2009,
                              end_year: int=2020,
                              region_name='SAT',
    ...
    '''
    print('start plotting...')
    fig, (ax2, ax3) = plt.subplots(2,1, figsize=(8,5), sharex=True, gridspec_kw={'height_ratios':[8,2.7]})
    fig.subplots_adjust(hspace=0)
    savename='MSC_with_bar_plot_precip_'
    first_month = start_month-2
    first_trendy=True
    j=0
    if compare_case==1 or compare_case==2:
        ...
        j=0
        for i, column in enumerate(df_for_bar_plot):
            print(i)
            df_precip_MSC = calculateMeanSeasonalCycle_of_fluxes(column, model='ERA5')
            df_precip_MSC.loc[:first_month,'Month'] += 12
            df_precip_MSC_new = pd.DataFrame(pd.concat([df_precip_MSC.loc[first_month+1:, 'Month'], df_precip_MSC.loc[:first_month, 'Month']], ignore_index=True))
            df_precip_MSC = pd.merge(df_precip_MSC_new, df_precip_MSC, on='Month', how='outer')
            a = ax3.bar(df_precip_MSC['Month'], (df_precip_MSC[column_to_plot_bar_plot[i]])*1000, bar_width, alpha=opacity, color='steelblue', label=columns_to_plot_label_bar_plot[i])
            j=j+1
        ax3.minorticks_on()
        ax3.grid(visible=True, which='minor', color='white', axis='y', linestyle='-', linewidth='0.4')
        ax3.tick_params(bottom=True, left=True, color='gray')
        ax3.set_xlabel('Month')
        ax3.set_ylabel('Mean precip\n[mm/day]', color='black', labelpad=15)
        ax3.legend(framealpha=0.5, fontsize=9.5, loc='upper left', ncol=1, columnspacing=0.6)
    ...
    ax2.get_xaxis().set_minor_locator(mpl.ticker.AutoMinorLocator())
    ax2.get_yaxis().set_minor_locator(mpl.ticker.AutoMinorLocator())
    ax2.tick_params(bottom=True, left=True, color='gray')
    if legend_next_to_plot:
        ax2.legend(framealpha=0.4, facecolor='grey', loc=location_of_legend, fontsize=9.5, ncol=ncol, columnspacing=0.6, handletextpad=0.4, bbox_to_anchor=(1, 0.2))
    else:
        ax2.legend(framealpha=0.4, facecolor='grey', loc=location_of_legend, fontsize=8.75, ncol=ncol, columnspacing=0.6, handletextpad=0.4)
