...
# plot maps as xarray
def plotMapForERA5_swvl1(gdf_ERA5: geopandas.GeoDataFrame, Year_or_Month_plotted: int=1, savepath: str='/home/lartelt/MA/software/'): 
    '''Documentation
    ...
    if Year_or_Month_plotted>12:
        ...
        df_ERA5_new = gdf_ERA5.groupby(['Year', 'latitude', 'longitude'])['swvl1'].sum().reset_index()
    else:
        what_is_plotted = 'Month'
        print('grouping ERA5 by Month...')
        df_ERA5_new = gdf_ERA5.groupby(['Month', 'latitude', 'longitude'])['swvl1'].sum().reset_index()
    savename='ERA5_soilmoisture_for_'+what_is_plotted+'_'+str(Year_or_Month_plotted)
    ...
    foo = xr.DataArray(xsdf['swvl1'][:])  # creates a DataArray with the xsdf data: ndarrays with values & labeled dimensions & coordinates
    ...
    foo.plot.contourf(x = 'longitude', y = 'latitude', extend = 'max', ax=ax, transform = ccrs.PlateCarree(), levels=256, cmap=cmap, vmin=0, vmax=6,
                      cbar_kwargs=dict(orientation='vertical', norm=norm, shrink=1, pad=0.03, 
                                       ticks=np.arange(0,6,0.5), label='soilmoisture [$m^3$/$m^3$/'+what_is_plotted+'/gridcell]'))
    ax.coastlines()
    ...
    ax.set_title('ERA5 soilmoisture for '+what_is_plotted+' '+str(Year_or_Month_plotted), fontsize=18)
    print('done plotting, now saving...')
