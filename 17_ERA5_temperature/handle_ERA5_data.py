...
for l, layer in enumerate(layers):
    ...
    ds = xr.open_mfdataset('/mnt/data/users/eschoema/ERA5/SouthAmerica/MonthlyTemperatures.nc')
    print('coarsen...')
    ...
    print(df.head())
    df.to_pickle('/mnt/data/users/lartelt/MA/ERA5_temperature/ERA5_1x1_grid/df_ERA5_MonthlyTemperature_per_gridcell_SA.pkl')
    print('df to gdf...')
    gdf = geopandas.GeoDataFrame(df, geometry=geopandas.points_from_xy(df.longitude, df.latitude), crs='EPSG:4326')
    gdf.to_pickle('/mnt/data/users/lartelt/MA/ERA5_temperature/ERA5_1x1_grid/ERA5_MonthlyTemperature_per_gridcell_SA.pkl')
    ...
    for i, region in enumerate(region_list): #for region in region_list_CT_Mask:
        ...
        if i==0 or i==1:
            ...
            gdf_SA = load_datasets.load_ERA5_temperature_gdf(region='SA', unit='per_gridcell')
        else:
            print('read gdf for '+region)
            gdf_SA = load_datasets.load_ERA5_temperature_gdf(region='SAT', unit='per_gridcell')
        ...
        gdf_subregion = cut_gridded_gdf_into_region_from_CT_Mask(gdf=gdf_SA, CT_Mask=CT_Mask, region_name=region, savepath='/mnt/data/users/lartelt/MA/ERA5_temperature/ERA5_1x1_grid/', savename='ERA5_MonthlyTemperature_per_gridcell_'+region)
        print(gdf_subregion.columns)
        plot_map_func.plotGDFspatially_simple_world(gdf=gdf_subregion, savepath_w_savename='/mnt/data/users/lartelt/MA/ERA5_temperature/ERA5_1x1_grid/map_ERA5_temperature_'+region+'.png')
        gdf_subregion_total_precip = calculate_subregional_total_flux(gdf=gdf_subregion, model='ERA5', layer=layer, to_unit='per_subregion', subregion=region, savepath='/mnt/data/users/lartelt/MA/ERA5_temperature/ERA5_1x1_grid/', savename='ERA5_MonthlyTemperature_per_subregion')
    ...
    for j, region in enumerate(region_list_arid_humid):
        ...
        for boundary_type in [strict_bound, loose_bound]:
            ...
            if j>1:
                ...
                gdf_ERA5 = load_datasets.load_ERA5_temperature_gdf(region='SAT_'+boundary+'limit_arid', unit='per_gridcell')
            else:
                gdf_ERA5 = load_datasets.load_ERA5_temperature_gdf(region='SAT', unit='per_gridcell')
            print('cut gdf_SA into humid/arid region')
            gdf_subregion = cut_gridded_gdf_into_region_from_CT_Mask(gdf=gdf_ERA5, CT_Mask=boundary_type, region_name=region, savepath='/mnt/data/users/lartelt/MA/ERA5_temperature/ERA5_1x1_grid/', savename='ERA5_MonthlyTemperature_per_gridcell_SAT_'+boundary+'limit_'+region)
            plot_map_func.plotGDFspatially_simple_world(gdf=gdf_subregion, savepath_w_savename='/mnt/data/users/lartelt/MA/ERA5_temperature/ERA5_1x1_grid/map_ERA5_temperature_'+boundary+'limit_'+region+'.png')
            ...
            gdf_subregion_total_precip = calculate_subregional_total_flux(gdf=gdf_subregion, model='ERA5', layer=layer, to_unit='per_subregion', subregion=region, 
                                                                        savepath='/mnt/data/users/lartelt/MA/ERA5_temperature/ERA5_1x1_grid/',
                                                                        savename='ERA5_MonthlyTemperature_per_subregion_SAT_'+boundary+'limit')
