...
def load_ERA5_temperature_gdf(region:str='SAT', unit:str='per_subregion', start_year:int=None, end_year:int=None):
    '''Documentation
    # arguments:
        - region: str
           - 'SAT', 'SATr', 'SAT_SATr'
    ...
    '''
    print('start loading ERA5 data...')
    path = '/mnt/data/users/lartelt/MA/ERA5_temperature/ERA5_1x1_grid/'
    gdf = pd.read_pickle(path+'ERA5_MonthlyTemperature_'+unit+'_'+region+'.pkl')
    if start_year!=None:
        gdf.drop(gdf[(gdf['Year'] < start_year)].index, inplace=True)
    if end_year!=None:
        gdf.drop(gdf[(gdf['Year'] > end_year)].index, inplace=True)
    print('Done loading data from '+path+'ERA5_MonthlyTemperature_'+unit+'_'+region+'.pkl')
    return gdf


def load_ERA5_evaporation_gdf(region:str='SAT', unit:str='per_subregion', start_year:int=None, end_year:int=None):
    '''Documentation
    # arguments:
        - region: str
           - 'SAT', 'SATr', 'SAT_SATr'
    ...
    '''
    print('start loading ERA5 data...')
    path = '/mnt/data/users/lartelt/MA/ERA5_evaporation/ERA5_1x1_grid/'
    gdf = pd.read_pickle(path+'ERA5_MonthlyEvaporation_'+unit+'_'+region+'.pkl')
    if start_year!=None:
        gdf.drop(gdf[(gdf['Year'] < start_year)].index, inplace=True)
    if end_year!=None:
        gdf.drop(gdf[(gdf['Year'] > end_year)].index, inplace=True)
    print('Done loading data from '+path+'ERA5_MonthlyEvaporation_'+unit+'_'+region+'.pkl')
    return gdf
