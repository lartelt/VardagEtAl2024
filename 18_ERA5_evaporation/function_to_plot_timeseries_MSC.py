...
def calculateMeanSeasonalCycle_of_fluxes(Flux_per_subregion_monthly_timeseries: pd.DataFrame, model: str='MIP', TRENDY_FLUXCOM_variable: str=None, 
    ...
    if model=='MIP' or model=='MIP_ens':
        ...
        Flux_msc = Flux_msc.merge(Flux_msc_count, on='Month', suffixes=('', '_count'))
        return Flux_msc
    elif model=='ERA5_evap' or model=='ERA5_evaporation':
        print('start calculating MSC for ERA5 evaporation')
        Flux_msc = Flux_per_subregion_monthly_timeseries.groupby(['Month'])[['e']].mean().reset_index()
        Flux_msc_std = Flux_per_subregion_monthly_timeseries.groupby(['Month'])[['e']].std(ddof=0).reset_index()
        Flux_msc_count = Flux_per_subregion_monthly_timeseries.groupby(['Month'])[['e']].count().reset_index()
        Flux_msc = Flux_msc.merge(Flux_msc_std, on='Month', suffixes=('', '_std'))
        Flux_msc = Flux_msc.merge(Flux_msc_count, on='Month', suffixes=('', '_count'))
        return Flux_msc
...
def plot_MSC_ERA5_precip_soilmoisture_temp(df_list: list=[],
                                  model_list: list=['ERA5_precip', 'ERA5_soil', 'ERA5_temp', 'ERA5_evap'],
                                  columns_to_plot: list=['tp_mean', 'swvl1_mean', 't2m', 'e'],
                                  columns_to_plot_std: list=['tp_std', 'swvl1_std', 't2m_std', 'e_std'],
                                  columns_to_plot_label: list=['precipitation', 'soilmoisture', 'temperature', 'evaporation'],
    ...
    ax3 = ax2.twinx()
    ax5 = ax2.twinx()
    ax5.spines.right.set_position(("axes", 1.135))
    #ax6 = ax2.twinx()
    #ax6.spines.right.set_position(("axes", 1.25))
    color_list=[]
    ...
    #lines_4, labels_4 = ax6.get_legend_handles_labels()
    ...
    ax2.set_ylabel('precipitation/evaporation [mm]', color=color_precip)
    ...
    '''ax6.set_ylabel('Evaporation [mm (water equiv.)]', color=color_evap)
    ax6.tick_params(axis='y', colors=color_evap)
    ax6.yaxis.label.set_color(color_evap)
    #ax6.set_ylim(12, 23)
    ax6.spines['right'].set_visible(True)
    ax6.spines['right'].set_color('dimgrey')
